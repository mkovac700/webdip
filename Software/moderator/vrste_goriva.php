<?php
    ...
    //$upit = "SELECT bpvg.id_bpvg, bpvg.id_benzinska_postaja, bpvg.id_vrste_goriva, bpvg.kolicina, pc.iznos, bpvg.na_raspolaganju 
    //FROM benzinska_postaja_vrste_goriva bpvg 
    //INNER JOIN povijest_cijena pc ON bpvg.id_bpvg = pc.id_bpvg 
    //WHERE bpvg.id_benzinska_postaja = '{$id_bp}'"; //WHERE bpvg.id_benzinska_postaja = '{$id_bp}'

    $upit = "SELECT bpvg.id_bpvg, vg.tip, vg.naziv, bpvg.kolicina, pc.iznos, bpvg.na_raspolaganju FROM vrste_goriva vg
             INNER JOIN benzinska_postaja_vrste_goriva bpvg ON vg.id_vrste_goriva = bpvg.id_vrste_goriva
             INNER JOIN povijest_cijena pc ON bpvg.id_bpvg = pc.id_bpvg 
             WHERE bpvg.id_benzinska_postaja = '{$id_bp}' AND pc.datum_do IS NULL";
    ...
    $smarty->display("vrste_goriva.tpl");
