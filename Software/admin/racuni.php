<?php
    ...
    if($_SESSION["uloga"] != 4){
        $upit = "INSERT INTO dnevnik (id_korisnik, id_tip_radnje, vrijeme, zapis) VALUES (NULL,3,'{$vrijeme}','Pokusaj pristupa stranici Uredivanje racuna ({$_SESSION["korisnik"]})')";
        ...
        $upit = "INSERT INTO dnevnik (id_korisnik, id_tip_radnje, vrijeme, zapis) VALUES ('{$id_korisnik}',3,'{$vrijeme}','Pristup stranici Uredivanje racuna ({$_SESSION["korisnik"]})')";
    ...
    if(isset($_POST['btn'])){
        ...
        if(isset($_POST['svi'])){
            $upit = "SELECT k.id_korisnik, tk.oznaka, k.korisnicko_ime, blokiran
                     FROM korisnik k INNER JOIN tip_korisnika tk
                     ON k.id_tip_korisnika = tk.id_tip_korisnika";
            //$upit = "SELECT id_korisnik, id_tip_korisnika, korisnicko_ime, blokiran FROM korisnik";
        }
        if(isset($_POST['blokirani'])){
            $upit = "SELECT k.id_korisnik, tk.oznaka, k.korisnicko_ime, k.blokiran
                     FROM korisnik k INNER JOIN tip_korisnika tk
                     ON k.id_tip_korisnika = tk.id_tip_korisnika
                     WHERE k.blokiran = 1";
